...
class CFormElement implements \ArrayAccess
{
    ...
    public function getHTML()
    {
        ...
        if (in_array($this['type'], ['submit', 'reset', 'button'])) {
            ...
            $html =  "<p><input id='$id' type='search'{$class}{$name}{$value}{$autofocus}{$required}{$readonly}{$placeholder}/><input id='do-{$id}' type='submit'{$classSubmit}{$label}{$readonly}{$title}/></p><p class='cf-desc'>{$description}</p>\n";        
            ...
            // checkbox
            $html =  "<p><input id='$id'{$type}{$class}{$name}{$value}{$autofocus}{$required}{$readonly}{$checked}{$title} /><label for='$id'>$label</label>{$messages}</p><p class='cf-desc'>{$description}</p>\n"; 
            ...
            $html =  "<div><p class='cf-label'>{$label}</p>{$ret}<p class='cf-desc'>{$description}</p></div>";
            ...
            $html =  "<div><p>{$label}</p>{$ret}<p class='cf-desc'>{$description}</p></div>";
        ...
        if( $this['render'] ) {
            if( is_callable($this['render'])) {
                $variables = compact(['messages', 'id', 'class', 'validates', 'name', 'label','autofocus','required', 'readonly','placeholder', 'multiple', 'max', 'min', 'low','high','optimum','step','size','text', 'checked', 'type','title','pattern','description','onlyValue','value']);
                return $this['render']($this, $html, $variables);
            }
        }
    ...
    public function validate($rules, $form)
    {
        ...
        $tests = [
          ...
          'not_empty' => array('message' => 'Får ej lämnas tomt', 'test' => 'return trim($value) != "";'),
